...
This example teaches **Field Scoped Rules** in isolation so the learner can see one bdd-tablex behavior without unrelated machinery competing for attention.
...
Path: `.\examples\cms\04_cell_dsl\05_field_scoped_rules`
...
```powershell
pytest examples/cms/04_cell_dsl/05_field_scoped_rules/test_field_scoped_rules.py -q
```
...
The README lives at `examples/cms/04_cell_dsl/05_field_scoped_rules/README.md`; the executable behavior lives next to it in the feature and test files.

## Previous / Next

Previous: [../04_fallbacks](../04_fallbacks)

Next: [../06_composed_dsls](../06_composed_dsls)
