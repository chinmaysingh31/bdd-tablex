...
This example teaches **Transformer Pipeline** in isolation so the learner can see one bdd-tablex behavior without unrelated machinery competing for attention.
...
Path: `.\examples\cms\05_transformers\06_transformer_pipeline`
...
```powershell
pytest examples/cms/05_transformers/06_transformer_pipeline/test_transformer_pipeline.py -q
```
...
The README lives at `examples/cms/05_transformers/06_transformer_pipeline/README.md`; the executable behavior lives next to it in the feature and test files.

## Previous / Next

Previous: [../05_custom_range_repeat_rules](../05_custom_range_repeat_rules)

Next: [../07_custom_transform_table_hook](../07_custom_transform_table_hook)
