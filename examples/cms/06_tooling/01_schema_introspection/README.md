...
This example teaches **Schema Introspection** in isolation so the learner can see one bdd-tablex behavior without unrelated machinery competing for attention.
...
Path: `.\examples\cms\06_tooling\01_schema_introspection`
...
```powershell
pytest examples/cms/06_tooling/01_schema_introspection/test_schema_introspection.py -q
```
...
The README lives at `examples/cms/06_tooling/01_schema_introspection/README.md`; the executable behavior lives next to it in the feature and test files.

## Previous / Next

Previous: [../../05_transformers/07_custom_transform_table_hook](../../05_transformers/07_custom_transform_table_hook)

Next: [../02_static_checker_python_api](../02_static_checker_python_api)
