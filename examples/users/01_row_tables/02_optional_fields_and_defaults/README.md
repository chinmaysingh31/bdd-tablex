...
This example teaches **Optional Fields And Defaults** in isolation so the learner can see one bdd-tablex behavior without unrelated machinery competing for attention.
...
Path: `.\examples\users\01_row_tables\02_optional_fields_and_defaults`
...
Feature: User focused example
  ...
  Scenario: Demonstrate Optional Fields and Defaults
...
```powershell
pytest examples/users/01_row_tables/02_optional_fields_and_defaults/test_optional_fields_and_defaults.py -q
```
...
The README lives at `examples/users/01_row_tables/02_optional_fields_and_defaults/README.md`; the executable behavior lives next to it in the feature and test files.
...
Source: `examples/users/01_row_tables/02_optional_fields_and_defaults/README.md`

## Previous / Next

Previous: [../01_basic_required_fields](../01_basic_required_fields)

Next: [../03_row_id_field_context](../03_row_id_field_context)
