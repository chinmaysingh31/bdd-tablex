# Row Id Field Context

## Purpose

This example teaches **Row Id Field Context** in isolation so the learner can see one bdd-tablex behavior without unrelated machinery competing for attention.
...
Path: `.\examples\users\01_row_tables\03_row_id_field_context`
...
```powershell
pytest examples/users/01_row_tables/03_row_id_field_context/test_row_id_field_context.py -q
```
...
The README lives at `examples/users/01_row_tables/03_row_id_field_context/README.md`; the executable behavior lives next to it in the feature and test files.

## Previous / Next

Previous: [../02_optional_fields_and_defaults](../02_optional_fields_and_defaults)

Next: [../04_row_id_diagnostics](../04_row_id_diagnostics)
