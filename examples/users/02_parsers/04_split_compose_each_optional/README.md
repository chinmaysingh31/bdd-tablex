# Split Compose Each Optional

## Purpose

This example teaches **Split Compose Each Optional** in isolation so the learner can see one bdd-tablex behavior without unrelated machinery competing for attention.
...
Path: `.\examples\users\02_parsers\04_split_compose_each_optional`
...
```powershell
pytest examples/users/02_parsers/04_split_compose_each_optional/test_split_compose_each_optional.py -q
```
...
The README lives at `examples/users/02_parsers/04_split_compose_each_optional/README.md`; the executable behavior lives next to it in the feature and test files.
...
Source: `examples/users/02_parsers/04_split_compose_each_optional/README.md`

## Previous / Next

Previous: [../03_choice_and_mapping](../03_choice_and_mapping)

Next: [../05_annotation_inference](../05_annotation_inference)
