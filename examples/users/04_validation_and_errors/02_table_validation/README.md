...
This example teaches **Table Validation** in isolation so the learner can see one bdd-tablex behavior without unrelated machinery competing for attention.
...
Path: `.\examples\users\04_validation_and_errors\02_table_validation`
...
Feature: User focused example
  ...
  Scenario: Demonstrate Table Validation
...
```powershell
pytest examples/users/04_validation_and_errors/02_table_validation/test_users_table_validation_example.py -q
```
...
The README lives at `examples/users/04_validation_and_errors/02_table_validation/README.md`; the executable behavior lives next to it in the feature and test files.

## Previous / Next

Previous: [../01_record_validation](../01_record_validation)

Next: [../03_source_aware_errors](../03_source_aware_errors)
