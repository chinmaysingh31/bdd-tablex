...
This example teaches **Source Aware Errors** in isolation so the learner can see one bdd-tablex behavior without unrelated machinery competing for attention.
...
Path: `.\examples\users\04_validation_and_errors\03_source_aware_errors`
...
Feature: User focused example
  ...
  Scenario: Demonstrate Source Aware Errors
...
```powershell
pytest examples/users/04_validation_and_errors/03_source_aware_errors/test_source_aware_errors.py -q
```
...
The README lives at `examples/users/04_validation_and_errors/03_source_aware_errors/README.md`; the executable behavior lives next to it in the feature and test files.

## Previous / Next

Previous: [../02_table_validation](../02_table_validation)

Next: [../04_collected_errors](../04_collected_errors)
