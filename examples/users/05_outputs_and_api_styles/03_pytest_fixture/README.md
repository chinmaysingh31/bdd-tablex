...
This example teaches **Pytest Fixture** in isolation so the learner can see one bdd-tablex behavior without unrelated machinery competing for attention.
...
Path: `.\examples\users\05_outputs_and_api_styles\03_pytest_fixture`
...
Feature: User focused example
  ...
  Scenario: Demonstrate Pytest Fixture API
...
```powershell
pytest examples/users/05_outputs_and_api_styles/03_pytest_fixture/test_pytest_fixture_api.py -q
```
...
The README lives at `examples/users/05_outputs_and_api_styles/03_pytest_fixture/README.md`; the executable behavior lives next to it in the feature and test files.
...
Source: `examples/users/05_outputs_and_api_styles/03_pytest_fixture/README.md`

Teaching note: this page is intentionally a little repetitive with nearby examples so each folder can stand alone in future docs.

Docs note: this folder remains intentionally standalone even though the learning path links it to its neighbors.

## Previous / Next

Previous: [../02_functional_api](../02_functional_api)

Next: [../04_dataclass_output](../04_dataclass_output)
