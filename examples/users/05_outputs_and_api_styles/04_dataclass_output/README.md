...
This example teaches **Dataclass Output** in isolation so the learner can see one bdd-tablex behavior without unrelated machinery competing for attention.
...
Path: `.\examples\users\05_outputs_and_api_styles\04_dataclass_output`
...
Feature: User focused example
  ...
  Scenario: Demonstrate Dataclass Output
...
```powershell
pytest examples/users/05_outputs_and_api_styles/04_dataclass_output/test_dataclass_output.py -q
```
...
The README lives at `examples/users/05_outputs_and_api_styles/04_dataclass_output/README.md`; the executable behavior lives next to it in the feature and test files.
...
Source: `examples/users/05_outputs_and_api_styles/04_dataclass_output/README.md`

## Previous / Next

Previous: [../03_pytest_fixture](../03_pytest_fixture)

Next: [../05_pydantic_output](../05_pydantic_output)
